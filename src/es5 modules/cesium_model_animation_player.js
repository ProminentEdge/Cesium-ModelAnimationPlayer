define([
    ...
    this.setTime = function(current_time) {
      ...
      for(var track_name in this.current_animation.tracks) {
        ...
        if(typeof curr_trans_keys !== 'undefined' && curr_trans_keys.length > 0) {
          ...
          var invMat = node.inv_rotation_matrix;
          ...
          if(curr_trans_keys[0].time == curr_trans_keys[1].time) {
            ...
            var start = new Cesium.Cartesian3(curr_trans_keys[0].value[0], curr_trans_keys[0].value[1], curr_trans_keys[0].value[2]);
            var end = new Cesium.Cartesian3(curr_trans_keys[1].value[0], curr_trans_keys[1].value[1], curr_trans_keys[1].value[2]);
        ...
        if(typeof curr_rot_keys !== 'undefined' && curr_rot_keys.length > 0) {
          ...
          var orig_inv = node.inv_rotation;
          var invMat = node.inv_rotation_matrix;
        ...
        if(typeof curr_scale_keys !== 'undefined' && curr_scale_keys.length > 0) {
          ...
          if(curr_scale_keys[0].time == curr_scale_keys[1].time) {
            ...
            var t = timeDelta/keyDelta;
            var start = new Cesium.Cartesian3(curr_scale_keys[0].value[0], curr_scale_keys[0].value[1], curr_scale_keys[0].value[2]);
            var end = new Cesium.Cartesian3(curr_scale_keys[1].value[0], curr_scale_keys[1].value[1], curr_scale_keys[1].value[2]);
            var result = new Cesium.Cartesian3();
            Cesium.Cartesian3.lerp(start, end, t, result);
  ...
  module.AnimationParser = function() {
    ...
    this._parseAnimationSetFromArrayBuffer = function(array_buffer) {
      ...
      for(var i = 0; i < animation_nodes.length; i++) {
        ...
        if(typeof nodes_dict[animation_nodes[i].name].rotation === 'undefined') {
          ...
          var orig_rot = nodes_dict[animation_nodes[i].name].rotation;
          var orig_quat = new Cesium.Quaternion(orig_rot[0], orig_rot[1], orig_rot[2], orig_rot[3]);
          var orig_quat_inv = new Cesium.Quaternion();
